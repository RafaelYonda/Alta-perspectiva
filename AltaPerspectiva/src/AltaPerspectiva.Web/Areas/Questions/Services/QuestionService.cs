...
namespace AltaPerspectiva.Web.Areas.Questions.Services
{
    ...
    {
        public QuestionViewModel GetQuestionViewModel(Question question, IQueryFactory queryFactory, IConfigurationRoot configuration, Guid loggedinUserId)
        {
            var q = question;
            ...
            qv.Answers = q.Answers.Where(drafted => drafted.IsDrafted != true && drafted.IsDeleted != true).Select(x => //Drafted is nullable .so only true are drafted
                               new AnswerViewModel
                               {
                                   ...
                                   //If loggedinuser fetch frma database to see the follower
        ...
        public List<QuestionViewModel> GetQuestionViewModels(IEnumerable<Question> questionList, IQueryFactory queryFactory, IConfigurationRoot configuration, Guid loggedinUserId)
        {
            AzureFileUploadHelper azureFileUploadHelper = new AzureFileUploadHelper();
            #region UserPrefetchOptimization
            List<Guid> userList = new List<Guid>();

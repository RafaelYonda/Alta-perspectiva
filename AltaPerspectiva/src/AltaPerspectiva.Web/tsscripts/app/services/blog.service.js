var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Injectable } from '@angular/core';
import { Http, Headers, Response, RequestOptions } from '@angular/http';
import { Observable } from 'rxjs/Observable';
import 'rxjs/add/operator/publishReplay';
var BlogService = (function () {
    function BlogService(_http) {
        this._http = _http;
        var headers = new Headers({ 'Content-Type': 'application/json' });
        headers.append('Authorization', 'Bearer ' + localStorage.getItem('auth_token'));
        var options = new RequestOptions({ headers: headers });
    }
    BlogService.prototype.IncreaseBlogPostViewCount = function (blogPost) {
        return this._http.post('blog/api/increaseblogpostviewcount/' + blogPost.id, blogPost)
            .map(this.extractData)
            .catch(this.handleError);
    };
    BlogService.prototype.addPostLike = function (postIdId, like) {
        return this._http.post('/blog/api/' + postIdId + '/addlike', like)
            .map(this.extractData)
            .catch(this.handleError);
    };
    BlogService.prototype.getBlogById = function (blogId) {
        return this._http.get('/blog/api/getblogbyid/' + blogId)
            .map(this.extractData)
            .catch(this.handleError)
            .publishReplay(1)
            .refCount();
    };
    BlogService.prototype.getPostLikeByPostId = function () {
        return this._http.get('/questions/api/categories/')
            .map(this.extractData)
            .catch(this.handleError)
            .publishReplay(1)
            .refCount();
    };
    BlogService.prototype.getPostAlreadyLiked = function (postId) {
        return this._http.get('/blog/api/' + postId + '/getalreadyLiked')
            .map(this.extractData)
            .catch(this.handleError)
            .publishReplay(1)
            .refCount();
    };
    BlogService.prototype.showLikeUserDetailsByBlogPost = function (postId) {
        return this._http.get('/questions/api/categories/')
            .map(this.extractData)
            .catch(this.handleError)
            .publishReplay(1)
            .refCount();
    };
    BlogService.prototype.addPostComment = function (postId, comment) {
        return this._http.post('/blog/api/' + postId + '/addcomment/', comment)
            .map(this.extractData)
            .catch(this.handleError);
    };
    BlogService.prototype.getPostCommentsByPostId = function (blogPostId) {
        return this._http.get('/blog/api/' + blogPostId + '/comments')
            .map(this.extractData)
            .catch(this.handleError)
            .publishReplay(1)
            .refCount();
    };
    BlogService.prototype.extractData = function (res) {
        var body;
        if (res.text()) {
            body = res.json();
        }
        return body || {};
    };
    BlogService.prototype.handleError = function (error) {
        var errMsg;
        if (error instanceof Response) {
            var body = error.json() || '';
            var err = body.error || JSON.stringify(body);
            errMsg = error.status + " - " + (error.statusText || '') + " " + err;
        }
        else {
            errMsg = error.message ? error.message : error.toString();
        }
        console.error(errMsg);
        return Observable.throw(errMsg);
    };
    return BlogService;
}());
BlogService = __decorate([
    Injectable(),
    __metadata("design:paramtypes", [Http])
], BlogService);
export { BlogService };
